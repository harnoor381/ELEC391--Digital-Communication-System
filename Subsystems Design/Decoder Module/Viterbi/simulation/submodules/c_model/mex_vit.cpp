...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[])
{
    ...
    std::string rcvsym_file = getString(v++, prhs);
    mexPrintf("rcvsym_file :%s\n", rcvsym_file.c_str());

    std::string blkstim_file = getString(v++, prhs);
    mexPrintf("blkstim_file :%s\n", blkstim_file.c_str());
    std::string dec_file = getString(v++, prhs);
    mexPrintf("dec_file :%s\n", dec_file.c_str());

    stim = read_stim_from_file(blkstim_file.c_str(), &nblocks);
    rcv_symbs = (int**)read_rcvsym_from_file(rcvsym_file.c_str(), nblocks, N.data(), stim);
    if (arch == "hybrid") {
    ...
    write_dec_to_file(dec_file.c_str(), decoded_table, nblocks, stim);
